...
class TestDataExport:
    def test_json_export_success(self, client, user, other_user):
        ...
        response = client.get("/api/users/me/export/?format=json")
        assert response.status_code == 200
        assert response['Content-Type'] == 'application/json'
        assert 'attachment; filename="data_export_exportuser.json"' in response['Content-Disposition']
    ...
    def test_unsupported_format(self, client, user):
        client.force_authenticate(user=user)
        response = client.get("/api/users/me/export/?format=xml")
