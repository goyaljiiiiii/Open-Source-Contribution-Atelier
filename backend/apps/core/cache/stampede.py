...
def stampede_protected_get_or_set(
    key, generate_func, timeout=300, beta=1.0, max_wait_ms=200
):
    """
    ...
    if acquired:
        ...
        while time.time() - wait_start < max_wait:
            ...
            if (
                cached_new is not None
                and isinstance(cached_new, dict)
                and "value" in cached_new
            ):
                # Only return if it's newer than our stale data (or if we had no stale data)
                if cached is None or cached_new.get("expiry", 0) > cached.get(
                    "expiry", 0
                ):
