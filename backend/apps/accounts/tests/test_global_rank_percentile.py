...
class TestGlobalRankPercentileStanding:
    def setup_method(self):
        ...
        self.user3 = User.objects.create_user(username="junior_dev", password="password")
        self.user4 = User.objects.create_user(username="newbie_dev", password="password")
        ...
        # user3: 200 XP (Rank 3)
        # user4: 50 XP (Rank 4)
        XPEvent.objects.create(user=self.user1, source_type="bonus", source_id=1, base_points=1000, multiplier=1.0, xp_delta=1000)
        XPEvent.objects.create(user=self.user2, source_type="bonus", source_id=2, base_points=500, multiplier=1.0, xp_delta=500)
        XPEvent.objects.create(user=self.user3, source_type="bonus", source_id=3, base_points=200, multiplier=1.0, xp_delta=200)
        XPEvent.objects.create(user=self.user4, source_type="bonus", source_id=4, base_points=50, multiplier=1.0, xp_delta=50)
    ...
    def test_single_user_percentile_fallback(self):
        solo_user = User.objects.create_user(username="solo_contributor", password="password")
    ...
    def test_tied_xp_rank_handling(self):
        ...
        XPEvent.objects.create(user=tie_user_a, source_type="bonus", source_id=10, base_points=300, multiplier=1.0, xp_delta=300)
        XPEvent.objects.create(user=tie_user_b, source_type="bonus", source_id=11, base_points=300, multiplier=1.0, xp_delta=300)
