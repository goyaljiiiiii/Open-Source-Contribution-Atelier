...
from apps.webhooks.tasks import dispatch_event, deliver_webhook, generate_signature
...
class TestWebhookCRUD:
    def test_create_webhook(self, api_client, user):
        ...
        response = api_client.post('/api/webhooks/endpoints/', {
    ...
    def test_update_webhook(self, api_client, user, endpoint):
        api_client.force_authenticate(user=user)
        response = api_client.patch(f'/api/webhooks/endpoints/{endpoint.id}/', {
            "is_active": False
        }, format='json')
...
class TestWebhookDelivery:
    ...
    def test_signature_validation(self):
        ...
        expected = hmac.new(b"supersecret", b'{"data":"test"}', hashlib.sha256).hexdigest()
        assert sig == expected
