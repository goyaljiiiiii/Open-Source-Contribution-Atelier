...
class ToxicityDetector:
    ...
    def _get_suggestions(self, score: float, text: str) -> list:
        ...
        if score > 0.8:
            suggestions.append(
                "Your comment contains potentially harmful language. Please rephrase respectfully."
            )
        ...
        if text.isupper():
            suggestions.append(
                "Avoid writing in all caps. It can be perceived as shouting."
            )

        if len(suggestions) == 0 and score > 0.7:
            suggestions.append(
                "Consider rephrasing your comment to be more constructive."
            )
