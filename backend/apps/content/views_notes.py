    ...
    def _get_or_create_lesson(self, lesson_id):
        ...
        if not lesson:
            ...
            lesson = Lesson.objects.create(
                slug=lesson_id, title=title, content="Lesson Content"
            )
        return lesson

    def get(self, request, lesson_id):
        lesson = Lesson.objects.filter(slug=lesson_id).first() or (
            Lesson.objects.filter(id=int(lesson_id)).first()
            if lesson_id.isdigit()
            else None
        )
        if lesson:
        ...
        return Response(
            {
                ...
                "updated_at": (
                    note.updated_at.isoformat() if note and note.updated_at else None
                ),
            }
        )
