...
def test_complete_upload_revalidates_size_against_limit(
    api_client, tmp_path, django_user_model
):
    ...
    with mock.patch(
        "apps.uploads.models.UploadSession.get_temp_dir", return_value=str(temp_dir)
    ):
        chunk_path = temp_dir / "0.part"
        chunk_path.write_bytes(b"x" * size_in_bytes)

        url = reverse(
            "uploads:complete_upload", kwargs={"session_id": session.session_id}
        )
