...
class TestBadgeSkipLoggingAndMetrics(TestCase):
    ...
    def test_award_specific_badge_missing_user_logs_warning_and_increments_metric(self):
        ...
        with self.assertLogs("apps.progress.tasks", level="WARNING") as cm:
            award_specific_badge(
                user_id=non_existent_user_id, badge_id=self.valid_badge.id
            )

        self.assertTrue(
            any(
                "Skipping badge award: Recipient user id=99999 not found" in log
    ...
    def test_award_specific_badge_missing_badge_logs_warning_and_increments_metric(
        self,
    ):
        ...
        with self.assertLogs("apps.progress.tasks", level="WARNING") as cm:
            award_specific_badge(
                user_id=self.valid_user.id, badge_id=non_existent_badge_id
            )

        self.assertTrue(
            any(
                "Skipping badge award: Badge id=88888 not found" in log
                for log in cm.output
            )
        )
    ...
    def test_evaluate_achievements_missing_user_logs_warning(self):
        ...
        self.assertTrue(
            any(
                "Skipping achievement evaluation: Recipient user id=77777 not found"
                in log
                for log in cm.output
            )
        )
    ...
    def test_award_badge_to_users_bulk_handles_missing_recipients(self):
        ...
        self.assertTrue(
            UserBadge.objects.filter(
                user=self.valid_user, badge=self.valid_badge
            ).exists()
        )
        self.assertTrue(
            any(
                "Skipping badge award for recipient user_id=55555" in log
                for log in cm.output
            )
        )
