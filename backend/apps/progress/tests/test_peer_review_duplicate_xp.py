...
def test_editing_peer_review_does_not_duplicate_xp():
    ...
    res1 = client.post(
        url, {"is_approved": True, "feedback": "Great work!"}, format="json"
    )
    assert res1.status_code == status.HTTP_201_CREATED

    initial_xp_count = XPEvent.objects.filter(
        user=reviewer, source_type="peer_review"
    ).count()
    assert initial_xp_count == 1

    # 2. Resubmit/edit review
    res2 = client.post(
        url, {"is_approved": True, "feedback": "Updated feedback!"}, format="json"
    )
    assert res2.status_code == status.HTTP_200_OK

    updated_xp_count = XPEvent.objects.filter(
        user=reviewer, source_type="peer_review"
    ).count()
