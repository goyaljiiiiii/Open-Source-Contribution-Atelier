...
class PRImpactAnalyzer:
    ...
    def parse_python_imports(self, code_content: str) -> Set[str]:
        ...
        try:
            ...
            found = re.findall(r"^(?:import|from)\s+([a-zA-Z0-9_]+)", code_content, re.MULTILINE)
        ...
        for filepath in changed_files:
            ...
            if norm_path.startswith("backend/apps/"):
                ...
                if len(parts) >= 3:
                    ...
                    affected_tests.add(f"backend/apps/{app_name}/tests_{parts[-1].replace('.py', '')}.py")
    ...
    def format_markdown_comment(
        ...
        if flaky_tests:
            ...
            for ft in flaky_tests:
                lines.append(f"- **`{ft['test_name']}`** — {ft['flaky_probability']}% failure probability ({ft['reason']})")
