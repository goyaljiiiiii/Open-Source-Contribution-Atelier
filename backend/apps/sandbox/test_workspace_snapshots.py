...
def setup_files(project):
    ProjectFile.objects.create(
        project=project, path="src/index.js", content="console.log('v1');"
    )
    ProjectFile.objects.create(
        project=project, path="src/utils.js", content="export const sum = (a,b) => a+b;"
    )
...
class TestWorkspaceSnapshotEdgeCases:
    ...
    def test_create_snapshot(self, api_client, user, project, setup_files):
        ...
        response = api_client.post(
            "/api/sandbox/workspace-snapshots/",
            {
    ...
    def test_cross_user_isolation(self, api_client, user, user2, project, setup_files):
        ...
        res3 = api_client.post(
            f"/api/sandbox/workspace-snapshots/{snapshot_id}/restore/"
        )
        assert (
            res3.status_code == 404
        )  # Should not be found since queryset filters by project__user
    ...
    def test_restore_edge_cases(self, api_client, user, project, setup_files):
        ...
        ProjectFile.objects.create(
            project=project, path="src/new_file.js", content="junk data"
        )  # User creates a junk file

        assert ProjectFile.objects.filter(project=project).count() == 2

        # 3. Restore the snapshot
        res_restore = api_client.post(
            f"/api/sandbox/workspace-snapshots/{snapshot_id}/restore/"
        )
        ...
        restored_utils = ProjectFile.objects.filter(
            project=project, path="src/utils.js"
        )
