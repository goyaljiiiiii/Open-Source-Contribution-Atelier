...
const CHANNELS = [
  {
    id: "chat",
    name: "Chat Room",
    url: "ws://localhost:8000/ws/chat/room-42/",
  },
  {
    id: "notifications",
  ...
  {
    id: "collab",
    name: "Live Code Collab",
    url: "ws://localhost:8000/ws/collab/session-789/",
  },
  {
    id: "sandbox",
    name: "Sandbox Terminal",
    url: "ws://localhost:8000/ws/sandbox/term-1/",
  },
];
    ...
    triggerAnimationSequence([
      {
        ...
        action: () => {
          addLog(
            "outbound",
            "ws_connect",
            { url: activeChannelObj.url, headers: { Upgrade: "websocket" } },
            "1. Client initiates HTTP 101 WebSocket Upgrade",
          );
        },
      },
      {
        ...
        action: () => {
          addLog(
            "system",
            "consumer_accept",
            { consumer: "ChatConsumer", scope: "websocket" },
            "2. Django Channels accepts connection & authenticates JWT",
          );
        },
      },
      {
        ...
        action: () => {
          addLog(
            "system",
            "channel_layer_group_add",
            {
              group: `chat_${selectedChannel}`,
              channel: "specific.channel!abc123",
            },
            "3. Registered consumer in Redis Channel Layer group",
          );
        },
      },
      {
        ...
        action: () => {
          ...
          addLog(
            ...
            { status: 101, message: "WebSocket connection ready" },
            "4. Handshake complete: ws:// connected",
          );
      ...
      },
      {
        ...
        action: () => {
          addLog(
            ...
            "2. Consumer calls disconnect(close_code=1000)",
          );
        },
      },
      {
        ...
        action: () => {
          addLog(
            ...
            "group_discard",
            { group: `chat_${selectedChannel}` },
            "3. Removed from Redis channel group",
          );
    ...
    triggerAnimationSequence([
      {
        ...
        action: () => {
          addLog(
            ...
            "chat_message",
            payload,
            "1. Outbound WS Text Frame sent by React hook",
          );
        },
      },
      {
        ...
        action: () => {
          addLog(
            "system",
            "consumer_receive",
            { method: "receive_json()", payload },
            "2. Consumer parses JSON and calls channel_layer.group_send()",
          );
        },
      },
      {
        ...
        action: () => {
          addLog(
            ...
            { group: `chat_${selectedChannel}`, event: "chat_message" },
            "3. Redis Channel Layer publishes event to all channel workers",
          );
        },
      },
      {
        ...
        action: () => {
          addLog(
            ...
            "4. Broadcast message delivered back to client(s)",
          );
    ...
    triggerAnimationSequence([
      {
        ...
        action: () => {
          addLog(
            ...
            "typing_indicator",
            payload,
            "1. Client emits typing state event",
          );
        },
      },
      {
        ...
        action: () => {
          addLog(
            ...
            "group_send_typing",
            { event: "user_typing" },
            "2. Consumer relays ephemeral typing event",
          );
        },
      },
      {
        ...
        action: () => {
          addLog(
            ...
            "user_typing",
            payload,
            "3. Ephemeral typing event received by channel subscribers",
          );
        },
      },
    ]);
      ...
      <div className="p-6 bg-[#0e111a] border border-gray-800 rounded-2xl flex flex-col gap-4 shadow-xl relative overflow-hidden">
        <div className="flex items-center justify-between border-b border-gray-800 pb-3">
          <div className="flex items-center gap-2 text-xs font-bold text-gray-300 uppercase tracking-wider">
            <Activity className="w-4 h-4 text-blue-400" /> Sequence Architecture
            Topology Diagram
          </div>
          {activeStep && (
      ...
      <div className="p-5 bg-[#0e111a] border border-gray-800 rounded-2xl flex flex-col gap-4 shadow-xl">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-800 pb-3">
          <div className="flex items-center gap-2 text-xs font-bold text-white">
            <Terminal className="w-4 h-4 text-emerald-400" /> Real-Time Frame
            Inspector Log
        ...
        >
          {filteredLogs.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-gray-500 text-xs gap-2">
              ...
              <span>
                No frames logged yet. Click CONNECT or SEND_MESSAGE to simulate
                WebSocket traffic.
              </span>
            </div>
          ) : (
