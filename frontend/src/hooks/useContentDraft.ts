...
export function useContentDraft(initialLessonId?: number) {
  ...
  const fetchModules = useCallback(async () => {
    try {
      setIsLoading(true);
      const res = await api
        .get<ModuleDraftData[]>("/content/modules/")
        .catch(() => null);
      if (res && res.data && res.data.length > 0) {
        ...
        const savedNotesMap = JSON.parse(
          localStorage.getItem("atelier_saved_notes_map") || "{}",
        );
        const currRes = await fetch("/content/curriculum.json").catch(
          () => null,
        );
        if (currRes && currRes.ok) {
          const currData = await currRes.json();
          const mappedModules: ModuleDraftData[] = (currData.modules || []).map(
            (m: any, idx: number) => ({
              id: m.id ? idx + 1 : idx + 1,
              ...
              lessons: (m.lessons || []).map((l: any, lIdx: number) => {
                ...
                const savedNote =
                  savedNotesMap[lesId] ||
                  (localStorage.getItem(`atelier_study_note_${lesId}`)
                    ? JSON.parse(
                        localStorage.getItem(`atelier_study_note_${lesId}`)!,
                      )
                    : null);
  ...
  const saveDraft = useCallback(async (draft: LessonDraftData) => {
    ...
    try {
      ...
      localStorage.setItem(
        "atelier_saved_notes_map",
        JSON.stringify(savedNotesMap),
      );
    } catch (err) {
