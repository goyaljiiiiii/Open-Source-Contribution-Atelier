...
export function OAuthClients() {
  ...
  const [showTestModal, setShowTestModal] = useState<OAuthClientData | null>(
    null,
  );
  ...
  const fetchClients = async () => {
    try {
      ...
      console.warn(
        "Failed to fetch backend OAuth clients, utilizing local fallback state:",
        err,
      );
      setClients(SAMPLE_CLIENTS);
    } finally {
    ...
    try {
      ...
      try {
        ...
        createdClient = {
          ...
          clientSecret:
            clientType === "confidential"
              ? `secret_${Math.random().toString(36).substring(2, 22)}`
              : undefined,
          clientType,
          redirectUris,
          allowedScopes,
      ...
      if (createdClient) {
        ...
        setName("");
        toast.success(
          `Registered application "${createdClient.name}" successfully!`,
        );
      }
    } catch (err) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs md:text-sm font-medium text-gray-600 dark:text-gray-400 mt-0.5">
                Register third-party OAuth 2.0 applications, configure redirect
                URIs, and issue API access credentials.
              </p>
            </div>
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="p-12 text-center bg-slate-50 dark:bg-slate-800/40 border border-gray-200 dark:border-slate-800 rounded-2xl flex flex-col items-center gap-3">
          ...
          <p className="text-xs text-gray-500 max-w-md">
            Register a web app, CLI tool, or native mobile app to issue Client
            ID and Secret credentials for OpenID Connect authorization.
          </p>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl max-w-lg w-full p-6 flex flex-col gap-5 shadow-2xl">
            <h2 className="text-xl font-black text-gray-900 dark:text-white flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-blue-500" /> Register OAuth
              Application
            </h2>
            ...
            <form onSubmit={handleCreateClient} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                >
                  ...
                  <option value="public">
                    Public (SPA / Native Mobile PKCE App)
                  </option>
                </select>
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl max-w-xl w-full p-6 flex flex-col gap-4 shadow-2xl">
            ...
            <div className="flex flex-col gap-3 text-xs">
              <p className="text-gray-600 dark:text-gray-400">
                Generated OAuth 2.0 Authorization Endpoint for{" "}
                <strong>{showTestModal.name}</strong>:
              </p>
              ...
              <div className="p-3 bg-slate-100 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 rounded-xl font-mono break-all text-[11px] text-blue-600 dark:text-blue-400">
                ...
                {showTestModal.clientId}&redirect_uri=
                {encodeURIComponent(
                  showTestModal.redirectUris[0] ||
                    "http://localhost:3000/callback",
                )}
                &scope=
                {encodeURIComponent(showTestModal.allowedScopes.join(" "))}
              ...
              <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl text-emerald-700 dark:text-emerald-300">
                ✓ Endpoint correctly formatted according to RFC 7636 PKCE
                standards.
              </div>
            </div>
          </div>
