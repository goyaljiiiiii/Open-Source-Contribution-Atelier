...
function generateIndex() {
            ...
            if (lesson.filePath) {
                ...
                if (fs.existsSync(mdPath)) {
                    ...
                    const flushParagraph = () => {
                        ...
                        if (content.length > 10) { // Only index substantial text
                            index.push({
                                id: `idx-${blockId++}`,
                                slug: lesson.slug,
                                title: lesson.title,
                    ...
                    for (const line of lines) {
                        ...
                        if (headingMatch) {
                            ...
                            currentHash = currentHeading.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
    ...
    console.log(`Successfully generated search index with ${index.length} entries at ${OUTPUT_PATH}`);
}
